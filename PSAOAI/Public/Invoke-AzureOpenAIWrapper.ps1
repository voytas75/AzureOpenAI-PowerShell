...
function Invoke-AzureOpenAIWrapper {
    ...
    begin {
        ...
        . $PSScriptRoot\AIEventAnalyzer\Invoke-AzureOpenAIChatCompletion.ps1
        . $PSScriptRoot\Invoke-AzureOpenAIDalle3.ps1
    ...
    process {    
        ...
        try {
            $chatOutput = Invoke-AzureOpenAIChatCompletion -APIVersion $ApiVersion -Endpoint "https://$serviceName.openai.azure.com" -Deployment $Deployment -User $User -Temperature $Temperature -N $N -FrequencyPenalty $FrequencyPenalty -PresencePenalty $PresencePenalty -TopP $TopP -Stop $Stop -Stream $false -OneTimeUserPrompt $prompt -SystemPromptFileName "$PSScriptRoot\prompts\ArtFusion2.txt"
